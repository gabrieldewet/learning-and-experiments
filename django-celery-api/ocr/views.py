...
async def ocr_job(request: Request):
    ...
    logger.info(f"Got OCR engine at {asyncio.get_event_loop().time() - start_time:.2f}s")
    ...
    if "path" in request.data:
        ...
        multiple_files = input_path.is_dir() and not request_serializer.validated_data["single_file"]
        logger.info(f"Validated path at {asyncio.get_event_loop().time() - start_time:.2f}s")
        # Start processing in the background
        asyncio.create_task(process_path(job.id, input_path, multiple_files, ocr_engine))
        logger.info(f"Created background task at {asyncio.get_event_loop().time() - start_time:.2f}s")

        # Return the job ID immediately
        response_serializer = JobSerializer(job)
        logger.info(f"Prepared response at {asyncio.get_event_loop().time() - start_time:.2f}s")
